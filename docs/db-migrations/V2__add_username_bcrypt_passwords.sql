...
UPDATE user
SET username = LOWER(REGEXP_REPLACE(SUBSTRING_INDEX(email, '@', 1), '[^a-zA-Z0-9_]', '_'))
...
UPDATE user u
    INNER JOIN (
        ...
        FROM user
    ) dup ON u.id = dup.id AND dup.rn > 1
...
VALUES ('admin', 'admin@system.local', 'admin',
        '$2a$10$Ho15GPUjj9ZW8tkPq1etYeCy4nXuJJXCbcvzEzz0p3uedR9ySBg3C',
        'ROLE_ADMIN')
ON DUPLICATE KEY UPDATE
    password_hash = '$2a$10$Ho15GPUjj9ZW8tkPq1etYeCy4nXuJJXCbcvzEzz0p3uedR9ySBg3C',
    roles         = 'ROLE_ADMIN';
...
-- 验证
SELECT id, name, username, LEFT(password_hash, 20) AS hash_prefix, roles FROM user LIMIT 5;
