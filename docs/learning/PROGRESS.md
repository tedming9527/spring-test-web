...
- 分支：`master`。
- 当前正式代码基线：`014ad79`；该提交删除临时并发故障注入，事务回调实现仍保留。
...
**当前唯一主课题：设计“锁协调层”和“事务更新层”的边界，避免等待Redis锁时占用数据库连接。**
...
- 当前代码在事务内先执行 `selectById()`，随后最多等待Redis锁900毫秒，等待期间可能持续占用数据库连接。
- 下一步需要拆分锁协调与事务更新边界，同时保证Redis锁直到数据库提交或回滚完成后才释放。

### Spring代理与同类自调用（已学习，尚未实现分层）
...
- 下一步实践方案已明确：`CategoryServiceImpl` 作为非事务锁协调层，再外部调用独立事务Bean执行数据库更新。

状态边界：本知识点为理论问答已学习；尚未完成两个Bean的代码拆分、编译和“等待Redis锁时事务未开启”的运行验收。
...
1. 设计并实现锁协调层与事务更新层的拆分，避免Redis锁等待发生在数据库事务内。
2. 将事务提交、回滚、并发旧值回写固化为JUnit 5自动化测试，删除 `TX_ROLLBACK_TEST` 开关。
3. 验证数据库查询时间超过5秒锁租约、数据库异常和缓存删除失败。
4. 进入容量实验：数据库连接池最大2、等待1秒、SQL占用2秒、5个并发请求。
